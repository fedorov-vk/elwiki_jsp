 ...
 * (optional)
 * <li>url - If you <i>don't</i> want to create a Javascript-enabled tag, you
 * can use this to make the tab look just the usual tag, but instead, it will
 * actually link to that page. This can be useful in certain cases where you
 * have something that you want to look like a part of a tag, but for example,
 * due to it being very big in size, don't want to include it as a part of the
...
public class TabTag extends BaseWikiTag {
	...
	private boolean handleAccesskey() {
		...
		if (pos > -1) {
			m_tabTitle = m_tabTitle.substring(0, pos) + "<span class='accesskey'>" + m_tabTitle.charAt(pos) + "</span>"
