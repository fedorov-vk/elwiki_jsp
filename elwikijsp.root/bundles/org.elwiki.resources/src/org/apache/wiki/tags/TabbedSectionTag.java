...
public class TabbedSectionTag extends BodyTagSupport {
	...
	/**
	 * Set the id of the default tab (the tab which should be shown when the page is
	 * first loaded).
	 * 
	...
	public boolean validateDefaultTab(String aTabId) {
		if (m_firstTabId == null)
			m_firstTabId = aTabId;
		if (aTabId.equals(m_defaultTabId))
			m_defaultTabFound = true;
	...
	/**
	 * Returns true, if the tab system is currently trying to figure out which is
	 * the default tab.
	 * 
	...
	/**
	 * The tabbed section iterates 3 time through the underlying Tab tags - first it
	 * identifies the default tab (displayed by default) - second it generates the
	 * tabmenu markup (displays all tab-titles) - finally it generates the content
	 * of each tab.
	 * 
	 * @return {@inheritDoc}
	 * @throws {@inheritDoc}
