...
/**
 * Pointer of this TreeViewBase implementation stores in the root element of DOM's tree container.
 */
TreeViewBase = new Class({
	...
	populateNode: function(node) {
		...
		if (node.isLeaf() && !node.options.forceParent) {
			...
			button.addEventListener("click", function(e) {
				...
				if (domElement == null) {
					console.warn("No TreeViewBase founded.");
					return;
				}
...
TreeView = new Class({
	Implements: [TreeViewBase, Options],
	...
	*/
	setRoot: function(_root) {
		if (instanceOf(this.root, TreeNode)) {
			this.root = _root;
		}
	},
...
var PagesTreeCreator = {
	...
	domready: function() {
		Wiki.jsonrpc("/pageshierarchyTracker/pages", ["fvk_value", 16], PagesTreeCreator.callback);
	},

	callback: function(result) {
		var root = new TreeNode("Pages Container");
		for (let item of result) {
			if (typeof item == "object") {
				PagesTreeCreator.parseJsonObject(item, root);
			}
		}
		new TreeView(root, "#SoftPagesContainer", {show_root: false});
	},

	parseJsonObject: function(obj, node) {
		let childNode = new TreeNode(obj.name);
		node.addChild(childNode);
		if (obj.hasOwnProperty('children')) { // && Array.isArray(obj.children)
			for (let item of obj.children) {
				PagesTreeCreator.parseJsonObject(item, childNode);
			}
		}
	},
