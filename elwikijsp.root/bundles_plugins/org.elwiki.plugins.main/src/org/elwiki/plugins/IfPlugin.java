...
/**
 * The IfPlugin allows parts of a WikiPage to be executed conditionally, and is intended as a
 * flexible way of customizing a page depending on certain conditions. Do not use it as a security
 * mechanism to conditionally hide content from users (use page ACLs for that).
 * 
 ...
 *
 * <p>
 * If any of them match, the body of the plugin is executed. You can negate the content by prefixing
 * it with a "!". For example, to greet all admins, put the following in your LeftMenu:
 * </p>
 * 
 * <pre>
 *  [{If group='Admin'
 ...
 *
 * <p>
 * Note that you can't use "!Jack|!Jill", because for Jack, !Jill matches; and for Jill, !Jack
 * matches. These are not regular expressions (though they might become so in the future).
 * <p>
 *
 * <p>
 ...
 *
 * <p>
 * The difference between "contains" and "is" is that "is" is always an exact match, whereas
 * "contains" just checks if a pattern is available.
 * </p>
 *
 * <p>
 ...
 * </pre>
 * <p>
 * With the same mechanism, it's also possible to test for the existence of a variable - just use
 * "var" instead of "page".
 * </p>
 * 
 * <p>
 ...
 *
 * <p>
 * The functional (method ifInclude), decision-making part of this plugin may be called from other
 * code (e.g., other plugins) since it is available as a static method
 * {@link #ifInclude(WikiContext,Map)}. Note that the plugin body may contain references to other
 * plugins.
 * </p>
 */
public class IfPlugin implements WikiPlugin, InitializablePlugin {
	...
	/**
	 * Returns a boolean result based on processing the WikiContext and parameter Map as according to
	 * the rules stated in the IfPlugin documentation.
	 *
