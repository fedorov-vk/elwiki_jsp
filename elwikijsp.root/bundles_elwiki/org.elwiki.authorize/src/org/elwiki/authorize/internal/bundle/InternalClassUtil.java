...
//import org.elwiki.utils.XmlUtil;
import org.jdom2.Element;

/**
 * Contains useful utilities for class file manipulation. This is a static class, so there is no need to
 * instantiate it.
 *
 * @since 2.1.29.
 * @deprecated Этот класс используется в JSPwiki (ClassUtil) для организации 'плагинов' средствами Java.
 */
public final class InternalClassUtil {

	private static final Logger log = Logger.getLogger(InternalClassUtil.class);
	/**
	 * The location of the classmappings.xml document. It will be searched for in the classpath. It's value is
	 * "{@value}".
	 */
	 ...
	 */
	static {
		List<Element> nodes = XmlUtil.parse(AuthorizePluginActivator.getDefault().getBundle(),
				MAPPINGS, "/classmappings/mapping");
	 ...
	 */
	private InternalClassUtil() {
	}

	/**
	 * Attempts to find a class from a collection of packages. This will first attempt to find the class based
	 * on just the className parameter, but should that fail, will iterate through the "packages" -list,
	 * prefixes the package name to the className, and then tries to find the class again.
	 *
	 * @param packages
	 ...
	 */
	public static Class<?> findClass(List<String> packages, List<String> externaljars, String className)
			throws ClassNotFoundException {
		if (!classLoaderSetup) {
	...
	/**
	 * Setup the plugin classloader. Check if there are external JARS to add via property
	 * {@link org.elwiki.core.plugin.wiki.api.engine.PluginManager#PROP_EXTERNALJARS}
	 *
	 * @return the classloader that can load classes from the configured external jars or ,if not specified,
	...
	private static ClassLoader setupClassLoader(List<String> externaljars) {
		...
		if (externaljars.size() == 0) {
			...
			return InternalClassUtil.class.getClassLoader();
		}
		...
		return new URLClassLoader(urls, InternalClassUtil.class.getClassLoader());
	}

	/**
	 *
	 * It will first attempt to instantiate the class directly from the className, and will then try to prefix
	 * it with the packageName.
	 *
	 * @param packageName
	 *            A package name (such as "org.apache.wiki.plugins").
	 * @param className
	...
	public static Class<?> findClass(String packageName, String className) throws ClassNotFoundException {
		try {
			return InternalClassUtil.class.getClassLoader().loadClass(className);
		} catch (ClassNotFoundException e) {
			return InternalClassUtil.class.getClassLoader().loadClass(packageName + "." + className);
		}
	}
	 ...
	 * 
	 * @param rootPackage
	 *            the base package. Can be {code null}.
	...
	public static List<String> classpathEntriesUnder(final String rootPackage) {
		...
		if (StringUtils.isNotEmpty(rootPackage)) {
			try {
				en = InternalClassUtil.class.getClassLoader().getResources(rootPackage);
			} catch (IOException e) {
	 ...
	 * {@link File} is a directory all files inside it are stored, otherwise the {@link File} itself is stored
	 * 
	 * @param results
	 *            collection in which the found entries are stored
	 * @param file
	 *            given {@link File} to search in.
	 * @param rootPackage
	 *            base package.
	 */
	 ...
	 *            collection in which the found entries are stored
	 * @param jurlcon
	 *            given {@link JarURLConnection} to search in.
	 * @param rootPackage
	 *            base package.
	 */
	...
	/**
	 * This method is used to locate and instantiate a mapped class. You may redefine anything in the resource
	 * file which is located in your classpath under the name
	 * <code>ClassUtil.MAPPINGS ({@value #MAPPINGS})</code>.
	 * <p>
	 * This is an extremely powerful system, which allows you to remap many of the JSPWiki core classes to
	 * your own class. Please read the documentation included in the default <code>{@value #MAPPINGS}</code>
	 * file to see how this method works.
	 * 
	 * @param requestedClass
	 *            The name of the class you wish to instantiate.
	...
	/**
	 * This method is used to locate and instantiate a mapped class. You may redefine anything in the resource
	 * file which is located in your classpath under the name <code>{@value #MAPPINGS}</code>.
	 * <p>
	 * This is an extremely powerful system, which allows you to remap many of the JSPWiki core classes to
	 * your own class. Please read the documentation included in the default <code>{@value #MAPPINGS}</code>
	 * file to see how this method works.
	 * <p>
	 * This method takes in an object array for the constructor arguments for classes which have more than two
	 * constructors.
	 * 
	 * @param requestedClass
	 *            The name of the class you wish to instantiate.
	 * @param initargs
	 *            The parameters to be passed to the constructor. May be <code>null</code>.
	 * @return An instantiated Object.
	 * @throws WikiException
	 *             If the class cannot be found or instantiated. The error is logged.
	 * @since 2.5.40
	 */
	...
	/**
	 * Finds a mapped class from the c_classMappings list. If there is no mappped class, will use the
	 * requestedClass.
	 * 
	 * @param requestedClass
