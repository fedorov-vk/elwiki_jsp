...
/**
 * <p>
 * Logs in a user by extracting authentication data from an Http servlet session. First, the module
 * tries to extract a Principal object out of the request directly using the servlet requests's
 * <code>getUserPrincipal()</code> method. If one is found, authentication succeeds. If there is no
 * Principal in the request, try calling <code>getRemoteUser()</code>. If the
 * <code>remoteUser</code> exists but the UserDatabase can't find a matching profile, a generic
 * WikiPrincipal is created with this value. If neither <code>userPrincipal</code> nor
 * <code>remoteUser</code> exist in the request, the login fails.
 * </p>
 * <p>
...
public class WebContainerLoginModule extends AbstractLoginModule {
	...
	public boolean login() throws LoginException {
		...
		try {
			// First, try to extract a Principal object out of the request directly.
